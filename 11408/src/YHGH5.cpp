...
int main(){
    ...
    for(int i = 1; i <= M; i++){
        work[i] = new Node(i);
        Edge* e = new Edge(work[i], dest, 1, 0);
        Edge* se = new Edge(dest, work[i], 0, 0);
        e->revEdge = se;
        se->revEdge = e;
    ...
    for(int i = 1; i <= N; i++){
        ...
        cin >> n;
        people[i] = new Node(i);
        Edge* r = new Edge(root, people[i], 1, 0);
        Edge* sr = new Edge(people[i], root, 0, 0);
        r->revEdge = sr;
        sr->revEdge = r;
        root->adj.push_back(r);
        people[i]->adj.push_back(sr);
        for(int j = 0; j < n; j++){
            int d, c;
            cin >> d >> c;
            Edge* e = new Edge(people[i], work[d], 1, c);
            Edge* se = new Edge(work[d], people[i], 0, -c);
            e->revEdge = se;
            se->revEdge = e;
            people[i]->adj.push_back(e);
            work[d]->adj.push_back(se);
        }
    }
    mcmf();
    cout << cnt << "\n" << ans;
